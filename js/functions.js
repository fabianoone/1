...
function fileListConseil(){
    fctBrowseDirectory("gamme_conseil") ;
    var html = "";
    if (env_Browse_Directory == 'Dossier non trouvé'){
    }
    else{
        ...
        for(var i = 0 ; i < jsArray.FILES.length ; i++){
            var fileName = jsArray.FILES[i].FILE;
            var fileNameSplitted = fileName.split(separator);
            html += "<li onClick=\"showDoc('gamme_conseil/"+encodeURI(fileName)+"')\"><span class=\"title\">"+fileNameSplitted[0]+"</span>&nbsp;&nbsp;<span class=\"subtitle\">"+fileNameSplitted[1].replace(".pdf","")+"</span></li>";
            //listing.push(new File(fileNameSplitted[0],fileNameSplitted[1]),fileName);
        }
    }

    $("#conseil-list").html(html);
}
...
function init_body(){
    ...
    /*$('.main').bind('touchstart',function(){console.log("laterSwipe");});
    $('.main').touchwipe(
                         {
                         wipeLeft: function() { setTimeout("swipe_event('next')",50); },
                         wipeRight: function() { setTimeout("swipe_event('prev')",50); },
                         min_move_x: 50,
                         min_move_y: 50,
